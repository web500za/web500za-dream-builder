...
export function PillNav({ currentSection, onSectionChange }: PillNavProps) {
  ...
  return (
    <div className="flex justify-center mt-1 mb-1 px-6">
      <div className="flex items-center rounded-full p-0.5 bg-white/20 backdrop-blur-sm">
        {sections.map((section, index) => (
          <div key={section.id} className="flex items-center">
            <button
              onClick={() => onSectionChange(section.id)}
              className={cn(
                "px-2 py-1.5 md:px-6 md:py-3 text-xs md:text-base font-medium transition-all duration-300 rounded-full whitespace-nowrap",
